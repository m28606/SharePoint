...
using TPCIP.PortalUserGuides.FakeData;

namespace TPCIP.PortalUserGuides.Fakes
{
    public class CustomerAgent : ICustomerAgent
    {
        private const string CUSTOMER_ID_TEST = "64715008";
        public event EventHandler updateNoteForAgentCalled;
        public event EventHandler deleteNoteForAgentCalled;
        ...
        private static List<AdditionalValue> _lastAdditionalValues = new List<AdditionalValue>(){  new AdditionalValue(){key = "ActualLidId",value = FakeCustomerIds.Customer01},
           new AdditionalValue()
        {
                key = "resumable",
                value = "false"
        ...
        }; //workaroud to persist state of last note
        ...
        /// <summary>
        /// In fake agents, GuideSessionId-s must refer to CustomerNote EntityId-s. This is helper function used to generate those ids.
        /// </summary>
        ...
        public virtual CustomerNote getCustomerNote(string id)
        {
            ...
            {
                throw new ArgumentException("Id has invalid format. It must be valid long number", "id");
            }

            var notes = _GetNotesMock(id == "2" ? CUSTOMER_ID_TEST : "64715009", "423", 0, 5, "");
            var note = notes.FirstOrDefault(n => n.id == _id) ?? notes.First();
            note.id = long.Parse(id);
            return note;
        }

        public virtual bool updateCustomerNote(CustomerNote customerNote)
        {
            if (customerNote.additionalValues != null)
            {
                _lastAdditionalValues = customerNote.additionalValues;
            }
            if (updateNoteForAgentCalled != null) updateNoteForAgentCalled(this, null);
